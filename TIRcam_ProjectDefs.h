#if !defined _ESP8266TIRCam_ProjectsDefs_
#define _ESP8266TIRCam_ProjectsDefs_
...
const char* camerStatech[] = { "camera idle", "camera waiting", "camera exposing", "cameras reading", "camera download", "camera error",  };
CameraStates cameraState = CAMERA_IDLE ;
CameraStates targetCameraState = CAMERA_IDLE ;

enum CameraCmds { CAM_NONE, CAM_EXP_START, CAM_EXP_STOP, CAM_EXP_ABORT };
...
//Image parameters
int bayerOffsetX, bayerOffsetY;
int binX, binY; 
int*** imageArray; //(colour plane, X - row, column )
int maxBinX = 4;                    // Returns the maximum allowed binning for the X camera axis  
int maxBinY = 4;                    //Returns the maximum allowed binning for the Y camera axis  
int numX = 0;                       //Sets the subframe width. Also returns the current value.  
int numY = 0;                       //Sets the subframe height. Also returns the current value.  
...
float CCDTemperatureSetpoint = 0.0F; // Sets the camera cooler setpoint in degrees Celsius, and returns the current setpoint.  
int CCDTemperature = 0;
bool coolerOn = false;
int coolerPower = 0; //%
...
#endif //_ESP8266TIRCam_ProjectsDefs_
